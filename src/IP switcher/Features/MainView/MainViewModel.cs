...
namespace Deucalion.IP_Switcher.Features.MainView
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        private string statusText = MainViewModelLoc.Status_None;
        ...
        public MainViewModel()
        {
            ...
            extractConfigToNewLocationCommand = new RelayCommand(() =>
                {
                    Effect = true;
                    var inputBox = new Deucalion.IP_Switcher.Features.InputBox.InputBoxView() { Owner = _Owner, WindowStartupLocation = WindowStartupLocation.CenterOwner };
                    inputBox.ShowDialog();
            ...
            createLocationCommand = new RelayCommand(() =>
                {
                    ...
                    LocationDetailView editLocationForm = new LocationDetailView(new Location.Location()) { Owner = _Owner, WindowStartupLocation = WindowStartupLocation.CenterOwner };
                    editLocationForm.ShowDialog();
                    if (editLocationForm.DialogResult ?? false)
                    {
            ...
            importPresetsCommand = new RelayCommand(() =>
                {
                    ...
                    {
                        DefaultExt = ".xml",
                        Filter = MainViewModelLoc.ExportFilter,
                        CheckPathExists = true,
                        AddExtension = true,
                        FileName = MainViewModelLoc.ExportDefaultFilename
                    };
                    ...
                    {
                        System.Windows.MessageBox.Show(String.Format(MainViewModelLoc.ErrorImportingLocations, Environment.NewLine, dialog.FileName, ex.Message));
                    }
        ...
        {
            ExternalIp = MainViewModelLoc.Searching;
            ...
            {
                publicIPAddress = MainViewModelLoc.SearchFailed;
            }
        ...
        {
            var location = new Location.Location() { Description = MainViewModelLoc.NewLocationDescription, ID = Settings.Default.GetNextID() };
        ...
        private void SetStatus(Status status)
        {
            ...
            {
                case Status.Idle:
                    StatusText = MainViewModelLoc.Status_Idle;
                    IsWorking = false;
                    break;
                case Status.ActivatingAdapter:
                    StatusText = MainViewModelLoc.Status_ActivatingAdapter;
                    IsWorking = true;
                    break;
                case Status.DeactivatingAdapter:
                    StatusText = MainViewModelLoc.Status_DeactivatingAdapter;
                    IsWorking = true;
                    break;
                case Status.ApplyingLocation:
                    StatusText = MainViewModelLoc.Status_ApplyingLocation;
                    IsWorking = true;
                    break;
                case Status.UpdatingAdapters:
                    StatusText = MainViewModelLoc.Status_UpdatingAdapters;
                    IsWorking = true;
                    break;
                default:
                    break;
